...
const mockUseSecCrawler = useSecCrawler as jest.MockedFunction<typeof useSecCrawler>;
...
describe('SecCrawlerManager', () => {
  ...
  describe('Crawl Configuration', () => {
    ...
    it('handles multiple form type selection', () => {
      ...
      const formTypeSelect = screen.getByLabelText('Form Types');
      fireEvent.change(formTypeSelect, { target: { value: ['10-K', '10-Q', '8-K'] } });
  ...
  describe('Advanced Configuration', () => {
    it('handles rate limit input', () => {
      ...
      const rateLimitInput = screen.getByLabelText('Rate Limit (requests/second)');
  ...
  describe('Crawl Execution', () => {
    ...
    it('handles crawl start error', async () => {
      const mockCrawlCompany = jest.fn().mockRejectedValue(new Error('Crawl failed'));
      mockUseSecCrawler.mockReturnValue({
        crawlCompany: mockCrawlCompany,
        importRssFeed: jest.fn(),
        isCrawling: false,
  ...
  describe('Crawl Results', () => {
    it('displays crawl results', () => {
      ...
      expect(screen.getByText('Processing Time: 45 seconds')).toBeInTheDocument();
    });
    ...
    it('displays warnings', () => {
      ...
      expect(screen.getByText('Some documents were skipped due to size limits')).toBeInTheDocument();
    });

    it('displays errors', () => {
      const resultWithErrors = {
        ...mockCrawlResult,
        errors: ['Network timeout', 'Invalid document format'],
      };
  ...
  describe('Error Handling', () => {
    it('displays crawl errors', () => {
      mockUseSecCrawler.mockReturnValue({
        ...
        progress: 0,
        status: 'error',
        error: 'Crawl failed: Network error',
      });
      ...
      expect(screen.getByText('Error: Crawl failed: Network error')).toBeInTheDocument();
    });
  ...
  describe('Performance', () => {
    it('handles large result sets efficiently', () => {
      ...
      expect(screen.getByText('Documents Processed: 10,000')).toBeInTheDocument();
