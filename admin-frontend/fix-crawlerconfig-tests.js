...
content = content.replace(/await user\.click\(([^)]+)\);/g, 'fireEvent.click($1);');

// Replace user.type() with fireEvent.change() for TextField components
content = content.replace(/await user\.clear\(([^)]+)\);\s*await user\.type\(([^)]+), "([^"]+)"\);/g, 'fireEvent.change($1, { target: { value: "$3" } });');
...
content = content.replace(/it\("([^"]+)", async \(\) => \{/g, 'it("$1", () => {');

fs.writeFileSync(testFile, content);

console.log('Fixed CrawlerConfig tests by replacing user interactions with fireEvent alternatives');
