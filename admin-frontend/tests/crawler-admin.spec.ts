...
test.describe("Crawler Admin Interface", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.locator("h1")).toContainText(
      "EconGraph Crawler Administration",
    );
  });
  ...
  test.describe("Dashboard Navigation", () => {
    test("should display main navigation tabs", async ({ page }) => {
      ...
      await expect(page.locator('[role="tab"]').nth(0)).toContainText(
        "Dashboard",
      );
  ...
  test.describe("Crawler Status Dashboard", () => {
    test("should display crawler status information", async ({ page }) => {
      ...
      const statusChip = page
        .locator('[data-testid="crawler-status-chip"]')
        .or(page.locator("text=Running").or(page.locator("text=Stopped")));
  ...
  test.describe("Configuration Management", () => {
    ...
    test("should allow configuration editing", async ({ page }) => {
      ...
      const maxWorkersField = page
        .locator('input[name="max_workers"]')
        .or(page.locator('input[placeholder*="workers" i]'));
    ...
    test("should display data sources table", async ({ page }) => {
      ...
      const addButton = page
        .locator('button:has-text("Add Data Source")')
        .or(page.locator('button:has-text("New Data Source")'));
...
test.describe("Visual Regression Tests", () => {
  ...
  test("configuration page should match expected appearance", async ({
    page,
  }) => {
    await page.goto("http://localhost:3001");
    await page.locator('[role="tab"]').nth(1).click();
