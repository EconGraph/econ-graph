...
test.describe('Crawler Admin Demo', () => {
  test('should demonstrate the complete crawler admin interface', async ({ page }) => {
    // Navigate to the admin interface
    await page.goto('http://localhost:3001');

    // Wait for the main app to load
    await expect(page.locator('[data-testid="dashboard-title"]')).toContainText('Crawler Administration');
    ...
    await expect(page.locator('[data-testid="crawler-status-card"]')).toBeVisible();
    await expect(page.locator('[data-testid="queue-statistics-card"]')).toBeVisible();
    await expect(page.locator('[data-testid="crawler-status-title"]')).toBeVisible();
    await expect(page.locator('[data-testid="queue-statistics-title"]')).toBeVisible();
    ...
    await expect(page.locator('[data-testid="global-settings-title"]')).toBeVisible();
    await expect(page.locator('[data-testid="data-sources-title"]')).toBeVisible();
    ...
    await page.screenshot({ path: 'test-results/controls-demo.png', fullPage: true });
  });
