...
- **Auth and permissions ([#176](https://github.com/EconGraph/econ-graph/pull/176)).**
  The backend uses fine-grained RBAC. It checks only `resource:action` roles carried
  in the token. Keycloak, adopted early in Phase 2, composes those roles into plans,
  organization roles and staff levels. The phases are:
  - 0 Hygiene, including CORS
...
The JWT secret, CORS and `/mcp` auth items are scheduled in Phase 0 (Hygiene) and
...
| `docs/technical/FULLTEXT_SEARCH.md` | Keep | Updated by [#165](https://github.com/EconGraph/econ-graph/pull/165) to describe search as built and list the gaps |
