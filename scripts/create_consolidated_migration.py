...
def extract_ddl_statements(schema_file: str) -> dict:
    ...
    # Extract indexes - simplified pattern
    index_pattern = r'CREATE INDEX (\w+) ON public\.(\w+) USING (\w+) \(([^)]+)\);'
    for match in re.finditer(index_pattern, content, re.DOTALL):
        index_name = match.group(1)
        table_name = match.group(2)
        index_type = match.group(3)
        columns = match.group(4)
        statements['indexes'].append((index_name, table_name, index_type, columns, ''))
...
def main():
    ...
    # Write to file
    output_file = "migrations/2025-02-01-000001_consolidated_initial_schema/up.sql"
