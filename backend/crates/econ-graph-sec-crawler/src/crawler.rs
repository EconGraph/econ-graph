    ...
    fn filter_filings<'a>(
        ...
        for filing in filings {
            ...
            if self.config.start_date.is_some() || self.config.end_date.is_some() {
                // One bad or missing date must not fail the whole company: skip that filing.
                let Ok(filing_date) = parse_sec_date(first(&filing.filing_date)) else {
                    warn!(
                        "Skipping filing {}: unparseable filing date {:?}",
                        first(&filing.accession_number),
                        first(&filing.filing_date)
                    );
                    continue;
                };
...
mod tests {
    ...
    #[test]
    fn filter_filings_skips_unparseable_dates_instead_of_failing() {
        ...
        let mut rows = subs.filings.recent.to_filing_infos();
        let good = rows.len() - 2;
        rows[0].filing_date = vec!["not-a-date".into()];
        rows[1].filing_date = vec![String::new()];
        for row in &mut rows {
            // Only the date filter is under test.
            row.is_xbrl = vec![1];
            row.size = vec![1];
        }
        ...
        let crawler = SecEdgarCrawler::with_fetcher(
            ...
            config.clone(),
            sec_http_fetcher(&config.user_agent, config.max_requests_per_second).unwrap(),
            SecEndpoints::new("http://data/", "http://archives"),
        );
        let kept = crawler.filter_filings(&rows).unwrap();
        assert_eq!(kept.len(), good);
        assert!(kept
            .iter()
            .all(|f| !f.filing_date[0].is_empty() && f.filing_date[0] != "not-a-date"));
    }
