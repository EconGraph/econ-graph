...
pub async fn get_series_data(
    ...
    // Revision filters run in SQL so pagination counts the rows actually returned.
    if let Some(as_of) = params.as_of {
        query = query.filter(data_points::revision_date.le(as_of)).filter(
            data_points::superseded_on
                .is_null()
                .or(data_points::superseded_on.gt(as_of)),
        );
    } else if params.latest_revision_only.unwrap_or(false) {
        query = query.filter(data_points::superseded_on.is_null());
    }
