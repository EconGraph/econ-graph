...
impl KeycloakAuthService {
    ...
    pub fn new() -> AppResult<Self> {
        let keycloak_url = env::var("KEYCLOAK_URL")
            .unwrap_or_else(|_| "http://auth.econ-graph.local".to_string());
        let realm = env::var("KEYCLOAK_REALM")
            .unwrap_or_else(|_| "econ-graph".to_string());
        let client_id = env::var("KEYCLOAK_CLIENT_ID")
            .unwrap_or_else(|_| "econ-graph-backend".to_string());
        let client_secret = env::var("KEYCLOAK_CLIENT_SECRET")
            .unwrap_or_else(|_| "backend-secret".to_string());
    ...
    async fn fetch_public_keys(&mut self) -> AppResult<HashMap<String, String>> {
        let url = format!("{}/realms/{}/protocol/openid-connect/certs", self.keycloak_url, self.realm);
        
        let response = self.http_client
            .get(&url)
            .send()
            .await
            .map_err(|e| AppError::AuthenticationError(format!("Failed to fetch Keycloak keys: {}", e)))?;

        if !response.status().is_success() {
            return Err(AppError::AuthenticationError(
                format!("Keycloak keys endpoint returned status: {}", response.status())
            ));
        }

        let keys_response: KeycloakPublicKeys = response
            .json()
            .await
            .map_err(|e| AppError::AuthenticationError(format!("Failed to parse Keycloak keys: {}", e)))?;
    ...
    fn jwk_to_pem(&self, jwk: &KeycloakPublicKey) -> AppResult<String> {
        ...
        let jwk_json = serde_json::to_string(jwk)
            .map_err(|e| AppError::AuthenticationError(format!("Failed to serialize JWK: {}", e)))?;
    ...
    pub async fn validate_token(&mut self, token: &str) -> AppResult<KeycloakClaims> {
        ...
        let kid = header.kid.ok_or_else(|| {
            AppError::AuthenticationError("Token missing key ID".to_string())
        })?;
        ...
        let public_key = public_keys.get(&kid)
            .ok_or_else(|| {
                AppError::AuthenticationError(format!("Unknown key ID: {}", kid))
            })?;
        ...
        let token_data = decode::<KeycloakClaims>(token, &DecodingKey::from_rsa_pem(public_key.as_bytes())?, &validation)
    ...
    pub fn claims_to_user(&self, claims: &KeycloakClaims) -> AppResult<User> {
        ...
        let organization = claims.additional_claims
            .get("organization")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());
    ...
    pub async fn get_service_token(&self) -> AppResult<String> {
        ...
        let response = self.http_client
            ...
            .send()
            .await
            .map_err(|e| AppError::AuthenticationError(format!("Failed to get service token: {}", e)))?;

        if !response.status().is_success() {
            return Err(AppError::AuthenticationError(
                format!("Service token request failed with status: {}", response.status())
            ));
        }
    ...
    pub async fn validate_service_token(&mut self, token: &str) -> AppResult<String> {
        ...
        if claims.aud == self.client_id {
            ...
            Err(AppError::AuthenticationError("Invalid service token".to_string()))
        }
    }
}
