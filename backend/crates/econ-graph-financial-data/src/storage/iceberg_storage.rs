...
pub struct PartitionCatalog {
    series_index: HashMap<Uuid, SeriesMetadata>,
...
impl PartitionCatalog {
    ...
    pub fn get_series(&self, series_id: &Uuid) -> Option<&SeriesMetadata> {
...
impl FinancialDataStorage for IcebergStorage {
    async fn write_series(&self, series: &EconomicSeries) -> Result<()> {
        ...
        // Note: In a real implementation, we'd persist this catalog
        tracing::info!("Adding series to catalog: {}", series.id);

        // For now, we'll create the data directory structure
        // The actual catalog persistence would be implemented later
        std::fs::create_dir_all(&self.data_dir)?;

        // The series metadata is stored in the catalog
        // Actual data points are written separately via write_data_points
        Ok(())
    }
