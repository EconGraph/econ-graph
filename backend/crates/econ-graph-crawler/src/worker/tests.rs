...
//! skipped when it is unset. They serialise on the crate-wide DB test lock
//! ([`crate::testkit::lock_test_db`]), so they are safe with or without `--test-threads=1`. Every row
//! they create uses external ids prefixed `t5_`; `crawl_queue` is emptied before each test.
...
async fn db() -> Option<Db> {
    let (url, guard) = crate::testkit::lock_test_db("worker").await?;
...
async fn lost_lease_is_reported_and_leaves_new_owner_alone() {
    ...
    for fail in [false, true] {
        ...
        assert_eq!(
            CrawlQueueItem::complete(&db.pool, &it).await.unwrap(),
